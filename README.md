...
The synchronous Rust port of
[`google/bumble`](https://github.com/google/bumble), the Python Bluetooth stack.

Bumble is a ~70,000-line dual-mode Bluetooth host stack plus a software
controller. This repository completed the port **one vertical slice at a
time**, with each slice a compiling, fully-tested Rust crate whose behavior is
verified against the upstream Python. Python-only asyncio scheduling is
represented by explicit synchronous polling, journals, listeners, and queues.
...
| 162. Typed scan advertisements | `bumble-host` | ✅ legacy/extended flags + per-address active/passive accumulation green |
| 163. Complete host link-status routing | `bumble-host` | ✅ Encryption Change V2 + key refresh + QoS + remote-host features green |
| 164. Device configuration loading | `bumble-host` | ✅ all typed defaults + JSON/file/special-field semantics + configured advertising green |
| 165. Configured device power-on | `bumble-host` + `bumble-controller` | ✅ identity/privacy + LE host features + Classic visibility/security + controller state green |
...
| 175. Device LE controller capability discovery | `bumble-host` | ✅ Supported Commands selection + all-page/legacy feature reads + PHY/advertising queries green |
| 176. Peer lookup by name and identity | `bumble-host` | ✅ owned LE/Classic discovery + EIR/AD matching + bonded-IRK RPA resolution green |
| 177. Upstream ISO regression sync | `bumble-host` | ✅ `bd64c127` zero-SDU PHY preservation + idempotent BIG/BIG-sync failure cleanup green |
| 178. Host flush and power teardown | `bumble-host` | ✅ ordered flush/disconnection events + complete connection/bearer/ACL cleanup green |
| 179. Classic remote-name lifecycle | `bumble-host` | ✅ address/connection requests + correlated HCI/UTF-8 failures + retained peer name green |
| 180. Host reset version and LMP discovery | `bumble-host` | ✅ reset/flush lifecycle + version + all-page/fallback LMP capabilities + scan gating green |
| 181. Host reset masks and controller packet pools | `bumble-host` + `bumble-transport` | ✅ version-aware Classic/LE masks + V2/V1 buffer discovery + Classic/LE/ISO flow queues green |
...
| 188. Serial DSR/DTR flow control | `bumble-transport` | ✅ asserted DTR + timeout-bounded DSR-gated writes green |
| 103–188. Repository completion audit | workspace | ✅ synchronous Rust port complete; full workspace gates green |
...
**Legend:** ✅ ported (complete for the synchronous Rust scope) · N/A means the
upstream surface is Python-runtime tooling rather than Bluetooth behavior.

Python awaitables and event-loop scheduling map to explicit synchronous polling,
result journals, listeners, and queues. LOC is the upstream module size, to
convey the audited surface.
...
| `controller.py` (2.8k) | `bumble-controller` | ✅ | **Complete upstream command surface**: every one of the 93 generated handlers plus the public Device-facing `LE Reject CIS Request`, `LE Read ISO TX Sync`, and local Channel Sounding capability paths gets the matching Command Complete/Status shape; all generated data commands provide their upstream state/default payloads; every stateful configuration handler retains its values; the remaining table fallbacks correspond exactly to upstream no-ops or TODO acknowledgements; and commands upstream also does not handle receive Unknown HCI Command. **Functionally simulated**: legacy, extended, and periodic LE advertising/scanning/synchronization; multi-set parameters/random addresses/fragmented data/scan responses; sync create/cancel/terminate, receive control, and PAST sync/set-info transfer over ACL; IRK resolving-list offload with identity-targeted RPA connections; ACL routing with PB/BC preservation and Number Of Completed Packets flow events; disconnection; event masks, Classic scan mode, default PHY, local name, suggested data length, synchronous flow control, mutable multi-page LE host features, and LE/SSP/Secure Connections host state; per-connection data length, PHY, LE subrate, and Sniff/Active mode changes; and — via LL control-PDU exchange — **encryption start**, **remote features**, and **CIS establishment or rejection**. CIG removal validates its identifier; CIS links retain Setup/Remove ISO data paths, route HCI ISO fragments with handle translation and completed-packet events, and expose the last routed SDU sequence/timestamp through Read ISO TX Sync. Reset clears CIS and TX-sync state. Classic connection/name/base and extended feature-page exchange, accept-time and explicit role switching, and SCO/eSCO request/accept/reject/disconnect are live. Upstream's own LTK-verification TODO and absent Classic-authentication/remote-version handlers remain the boundary. |
...
| `host.py` (2.1k) | `bumble-host` | ✅ | `Device` glue (ATT/EATT↔L2CAP↔ACL sequencing + pairing transport), including a configured handle-keyed SMP manager that routes fixed-channel PDUs, starts LE encryption, answers LTK requests, advances key distribution, and emits typed pairing completion/failure events. Controller-buffer-sized outbound ACL fragmentation, per-connection inbound reassembly, separate Classic/LE/ISO `DataPacketQueue` pools driven by Number Of Completed Packets, handle-indexed LE and Classic connection ownership, live LE signaling/credit-channel managers, LE/Classic encryption and remote-feature completion routing, resolving-list programming, Classic and LE L2CAP, Channel Sounding completion/result-fragment routing, opaque vendor-event publication, plus connected and broadcast ISO audio APIs are live. Reset-time discovery retains Supported Commands, local HCI/LMP version, all advertised LMP feature pages with legacy fallback, and the complete/legacy LE feature catalog; it then installs the upstream Classic, optional page-2, and version-compatible LE event masks, selects V2 or V1 LE/ISO buffers, falls back to the Classic ACL pool when LE reports zero capacity, reconciles the controller's suggested default data length to 251/2120, and retains extended-advertising set/data capacities with upstream's non-fatal 0/31 fallbacks. External-controller initialization applies the same reset tail and three-pool model atomically instead of collapsing separate LE capacity into the Classic queue or dropping ISO capacity. A transport-wide flush accounts for every queued/in-flight ACL/ISO packet and removes LE/Classic/CIS/SCO, ATT/EATT bearer, channel, pairing, encryption, remote-name requests, and pending-control state before correlated zero-reason disconnection events. Fixed ATT and EATT requests share the server while retaining bearer-scoped context, inboxes, CCCDs, MTUs, queued writes, and indication confirmations. LE connection update/rate/subrate, data-length, PHY, RSSI, CIG/CIS, ISO data-path, ISO TX-sync, and Classic remote-name completions update per-handle state and enter correlated result journals, including status-only failures. A typed `DeviceEvent` journal and synchronous listener registry route flush, LE/Classic/SCO lifecycle, raw and accumulated scan results, discovery, remote-name success/failure, pairing, V1/V2 encryption change, key refresh, QoS, remote-host features, Channel Sounding result fragments, opaque vendor events, and connection-control outcomes after state mutation; failed encryption and disconnection events preserve all per-handle state. The host pump advances LL, LMP, periodic-sync-transfer, BIG termination, and HCI/ACL traffic. Python awaitable conveniences map to explicit result journals, listeners, and synchronous drive methods. |
| `device.py` (7.0k) | `bumble-host` | ✅ | The full typed `DeviceConfiguration` default field set loads from JSON values/strings/files with upstream address, IRK, advertising-data, interval-alias, GATT-service, pairing, and dynamic-extra-key semantics; configured constructors, legacy advertising, and `power_on` consume it. Configured construction materializes custom services/characteristics/descriptors with upstream property and permission strings, appends enabled GAP/GATT defaults with a bound database hash, registers EATT when requested, and builds the live SMP manager from I/O capability, identity-address type, and debug-mode settings. Power-on resets the controller, learns its public address, Supported Commands, local version, all-page/fallback LMP features, and all-page/fallback LE features, generates/programs a missing static identity or privacy RPA, enables configured address resolution and LE host features, and applies Classic name/class/SSP/Secure Connections/EIR/visibility state; interlaced scan modes are applied after successful Host initialization and only when their individual LMP features are known. Retained feature bytes drive upstream-equivalent PHY and extended/periodic-advertising capability helpers. `flush`, `reset`, re-power, `power_off`, `update_rpa`, and observable address/power state expose the synchronous lifecycle surface; flush listeners run before all connection state is invalidated, then receive one disconnection per removed handle. High-level legacy, extended, and periodic LE advertising; active/passive raw scan reports plus upstream-equivalent typed `Advertisement` flag/radio metadata and per-address scan-response accumulation; name lookup owns legacy/extended scanning or Classic inquiry only when needed, matches Complete/Shortened Local Name data, and returns current RPAs by resolving bonded identity keys; periodic sync create/cancel/terminate, fragmented-report assembly, and PAST sync/set-info transfer; identity/RPA-aware legacy and extended connection setup; handle-indexed LE/Classic peer, role, connection parameters/rate/subrate, negotiated data length, PHY and RSSI, remote LE and multi-page LMP features, Sniff/Active, and Channel Sounding capability/config/procedure state; and disconnect run through `Device` without raw HCI. Classic remote-name requests accept either an address or established handle, use upstream's R2 paging defaults, correlate immediate command rejection and final HCI failure, reject malformed UTF-8, and retain successful names on matching connection state. `add_event_listener`/`remove_event_listener` and the ordered `take_device_events` journal provide the upstream lifecycle/discovery listener surface without forcing an async runtime. The upstream update, Bluetooth 6.2 rate, data-length, PHY read/set/default, RSSI, default-rate, and default-subrate conveniences all have handle-safe command APIs; explicit result events replace their futures. Extended and periodic data fragment across HCI commands up to the controller's 1650-byte limit. ATT, EATT, L2CAP, encryption, PAST, and ISO operations have explicit handle-selecting forms while legacy convenience calls use a selectable current connection. CIG/CIS exposes every group and directional QoS field, upstream defaults and zero-SDU RTN normalization while preserving PHY, batched creation, accept/reject, failure correlation, and established timing/link state; BIG/BIS synchronization reserves pending handles and releases them idempotently on command-status or establishment failure, while arbitrary data-path IDs/codecs/configuration/controller delays, installed-path state, TX-sync metadata, sequence numbering, controller-sized ISO fragmentation/flow control, and receive-side SDU reassembly are live. GATT/ATT, SMP, Classic, and synchronous operations are also exposed by the same type. Python awaitable conveniences map to the same explicit synchronous lifecycle surface. |
...
| `smp.py` (2.0k), `pairing.py` (0.3k) | `bumble-smp` | ✅ | Complete PDU codec and synchronous protocol behavior: Legacy and SC sessions cover every association model through encryption; responder-first phase 3 retains LTK/IRK/CSRK/Link Key material and counters; h6/h7 CTKD runs over LE and encrypted BR/EDR; bonds drive Security Request reconnect, privacy resolution, and signed ATT; and the handle-keyed manager owns concurrent session lifecycle while reusing one generated ECC key or selecting the Bluetooth fixed debug key. RPA generation and direct IRK verification are public helpers. Keypress Notification is codec-complete, matching upstream Bumble whose live session leaves `keypress = False`. |

### Transports & drivers
| Upstream | Rust crate | Status | Notes |
|---|---|---|---|
| `transport/*` — USB, UART/serial, TCP, WebSocket, UDP, PTY, android-netsim, vhci, … | `bumble-transport` | ✅ | Incremental H4 framing accepts fragmented/coalesced streams and vendor packet layouts. Bumble transport-name/metadata dispatch opens every upstream scheme: file, serial/UART, raw PTY, TCP client/server, UDP, Unix client/server, WebSocket client/server, Linux VHCI/raw HCI user-channel sockets, libusb/pyusb Bluetooth-controller endpoints, Android emulator gRPC, and Android netsim host/controller packet streams. Serial supports RTS/CTS plus DSR/DTR through asserted DTR and timeout-bounded DSR-gated writes. The typed HCI bridge pumps both directions with packet replacement, sender short-circuit responses, and trace hooks; `HciCommandChannel` correlates synchronous command responses while preserving unrelated traffic. `ExternalHost` resets controllers, discovers version plus the all-page LE feature catalog (with legacy eight-byte fallback) and base-or-multi-page LMP features, installs both event-mask pages, configures distinct ACL/ISO flow-control pools, reconciles suggested default data length, retains extended-advertising capacities, serializes Device commands by controller credit, and propagates terminal transport state into the Host flush lifecycle. BTSnoop/PCAP writers can wrap any bidirectional transport, and the bounded BTSnoop reader handles H1/H4 captures, timestamps, drops, and truncation. USB covers discovery, class/forced interface selection, commands, events, ACL, outgoing ISO-over-bulk compatibility, and `+sco=` SCO/eSCO isochronous input/output through the locked libusb ABI. |
...
- `SerialConfig` matches Bumble's 1 Mbaud default, optional numeric speed,
  ...
  RTS/CTS and asserts DTR. At this slice, DSR/DTR flow control still returned an
  explicit unsupported error; Slice 188 closes that historical gap with a
  timeout-bounded DSR gate.
...
The main upstream transport catalog was now represented. The then-remaining
USB SCO/isochronous input path was completed in Slice 106, and Slice 188 closes
the serial DSR/DTR path.
...
## Slice 162 — what's here

The host/device audit found that Rust retained raw HCI advertising reports but
did not expose upstream's `Advertisement`, `LegacyAdvertisement`,
`ExtendedAdvertisement`, or `AdvertisementDataAccumulator` behavior.

- `Advertisement` now preserves address and radio metadata plus every upstream
  legacy/extended flag: anonymous, legacy-PDU, connectable, directed,
  scannable, scan-response, complete, and truncated.
- `AdvertisementDataAccumulator` implements the exact active/passive scan
  rules. Active scanning waits for a scan response from each address and parses
  the concatenated advertising and response payloads; passive scanning emits a
  scannable advertisement immediately, and a second advertisement is emitted
  if the prior active request never received its response.
- The upstream observable split is retained deliberately: a combined result's
  typed `AdvertisingData` contains both payloads, while `data_bytes` remains the
  scan-response bytes.
- `Device` resets per-address accumulators when scanning starts, keeps the raw
  report queues/events for compatibility, and adds a high-level advertisement
  queue and `DeviceEvent::Advertisement` listener event.
- Focused tests pin legacy and extended flag conversion, anonymous/truncated
  metadata, active merge, passive delivery, and a live extended
  advertisement/scan-response pair through the local controller.

## Slice 163 — what's here

The host event audit found four upstream event families that were already
decoded by `bumble-hci` but silently discarded by `Device::poll`, plus one
failure-path state bug in the existing Encryption Change handler.

- Encryption Change V1 and V2 now share one typed device event. Per-handle LE
  and Classic state retains the enabled mode and V2 key size before listeners
  run; V1 uses upstream's zero key-size value.
- Failed encryption-change events no longer clear a previously encrypted link.
  Upstream emits a failure without mutating the connection, and Rust now does
  the same.
- Encryption Key Refresh Complete and QoS Setup Complete route distinct
  success/failure events. Successful QoS setup retains the service type while
  failures leave the last successful value intact.
- Remote Host Supported Features notifications are emitted even without a
  matching connection and are also retained on a matching Classic connection.
- A scripted host test covers initial encrypted Classic connection state, V2
  success, every new event, and failure-state preservation.

## Slice 164 — what's here

The `device.py` audit found that upstream's reusable `DeviceConfiguration` was
absent. Rust applications therefore had to duplicate identity, feature, and
advertising defaults instead of loading the same JSON files as Bumble.

- `DeviceConfiguration` exposes the complete upstream field set and defaults,
  including LE/Classic feature switches, privacy/offload options, service
  switches, pairing capability, L2CAP feature flags, GATT-service definitions,
  advertising data, and legacy interval values.
- JSON value/string and file constructors reproduce upstream's special loading
  order: addresses are typed as random unless qualified, explicit IRKs and AD
  payloads decode from hex, non-default addresses synthesize deterministic
  IRKs, default addresses receive a secure random IRK, and the legacy single
  interval is applied before min/max overrides.
- Changing only the name regenerates Complete Local Name advertising data;
  unknown dictionary keys are retained in `extra`, preserving the extensible
  Python configuration surface without weakening typed known fields.
- `Device::from_config`, `from_config_file`, and `with_server_and_config` retain
  the loaded configuration. `start_configured_advertising` sends its data,
  scan response, and millisecond-to-HCI interval conversion through any host
  transport.
- Focused tests pin every default family, address-derived and explicit IRKs,
  interval precedence, dynamic keys, validation errors, file construction, and
  exact live HCI commands.

## Slice 165 — what's here

Loading a configuration is now connected to upstream's controller lifecycle
instead of stopping at construction and advertising.

- `Device::power_on` validates the complete command payload before emitting
  anything, resets the controller, requests its public address, writes LE host
  support, and exposes powered/static/random/public address state.
- The all-zero random identity receives an upstream-style Random Static Address.
  Privacy-enabled devices generate a spec-valid RPA from the configured IRK;
  `update_rpa` provides runtime-neutral rotation and programs the new address.
- Configured address-resolution offload and CIS, subrate, Channel Sounding, and
  shorter-interval host-support bits are applied in upstream order.
- Classic power-on writes the padded local name, 24-bit Class of Device, SSP and
  Secure Connections host support, synthesized Extended Inquiry Response,
  connectable/discoverable scan flags, and interlaced scan modes.
- The software controller now retains and reports Class of Device, EIR, Secure
  Connections, scan modes, and mutable multi-page LE host features. An
  end-to-end LocalLink test proves the high-level commands alter live controller
  state and that `HCI_Read_BD_ADDR` updates the device.
- Atomic validation tests cover malformed IRKs and out-of-range Class of Device;
  exact capture tests pin default, privacy/feature, Classic, and RPA-rotation
  command sequences.

## Slice 166 — what's here

The Channel Sounding branch of configured power-on now completes instead of
ending at an untyped Command Complete packet.

- `ReturnParameters::LeCsReadLocalSupportedCapabilities` models every upstream
  field and its exact little-endian wire order, including NADM, timing masks,
  PHY support, switch time, and SNR capability.
- Success payloads round-trip through HCI serialization/parsing; the existing
  typed-status rule still collapses failed reads to a status-only result.
- The software controller advertises Channel Sounding in its LE feature page
  and returns a coherent four-configuration local capability set.
- `Device::poll` correlates the power-on capability read, retains both its
  completion status and typed local capabilities, and clears stale values on a
  new power-on attempt.
- Codec, controller query, remote-feature exchange, and LocalLink power-on tests
  prove the feature bit and capability data survive every layer.

## Slice 167 — what's here

Configured GATT definitions now become a live server instead of remaining raw
JSON attached to `DeviceConfiguration`.

- `Device::from_config` and `from_config_file` build the configured ATT/GATT
  server and report invalid service definitions as structured configuration
  errors rather than accepting an inert value.
- Custom services, characteristics, and descriptors preserve upstream UUID,
  property-name, permission-name, ordering, and legacy `permission`-key error
  semantics. Numeric permission masks remain accepted as upstream does.
- Enabled Generic Access and Generic Attribute services are appended after
  custom services. The GAP name and appearance values, GATT Service Changed,
  Client/Server Supported Features, and bound Database Hash use the existing
  profile and database implementations.
- `eatt_enabled` advertises EATT support and registers the assigned EATT SPSM
  for existing and future LE connections.
- Focused tests cover path-specific malformed definitions and prove configured
  fixed-ATT write/read, descriptor reads, default-service discovery, and EATT
  connection acceptance over a live `LocalLink`.

## Slice 168 — what's here

Configured pairing fields now drive live SMP behavior instead of remaining
inert `DeviceConfiguration` values.

- Configured devices own the existing handle-keyed `PairingManager`; connection
  establishment and teardown register and remove its per-handle state.
- `io_capability` and `identity_address_type` are validated and mapped into the
  pairing configuration. Invalid numeric values fail configured construction
  with field-specific errors.
- Incoming SMP fixed-channel traffic is parsed and routed automatically. The
  central starts encryption when phase 2 finishes, the peer answers the LTK
  request, and both managers advance encrypted key distribution to completion.
- `pair`/`pair_on_handle`, state/key/failure accessors, and a drainable error
  journal expose the synchronous control surface. `DeviceEvent` now reports
  typed pairing completion and failure.
- The manager reuses its generated P-256 key across sessions, matching upstream,
  and `smp_debug_mode` selects the Bluetooth specification's fixed debug key.
...
## Slice 175 — what's here

`Device` now learns and exposes the controller's local LE capability bitmap,
matching the upstream `Host`/`Device` feature-query boundary.

- Power-on reads the 64-byte Supported Commands bitmap and selects LE Read All
  Local Supported Features when advertised, with the legacy eight-byte command
  as the compatibility fallback.
- The selected completion status, raw feature bytes, and all-page maximum page
  remain observable. Failed or unsupported queries never manufacture support.
- `supports_le_feature(s)`, `supports_le_phy`, and the extended/periodic
  advertising helpers derive their answers from the learned controller bits;
  LE 1M remains mandatory and invalid PHY identifiers return a typed error.
- A live software-controller test proves multi-page host feature retention and
  the 248-byte path. Scripted older-controller tests prove fallback selection
  and the no-supported-query boundary.

## Slice 176 — what's here

Upstream's `find_peer_by_name` and `find_peer_by_identity_address` now have an
explicit, runtime-neutral `Device` lifecycle rather than requiring callers to
rebuild discovery and privacy matching around raw events.

- Each lookup receives a stable identifier and completes through a correlated
  result journal plus `DeviceEvent::PeerFound`. Multiple lookups can share one
  scan/inquiry, cancel independently, and stop lookup-owned activity only after
  its final consumer finishes.
- LE name lookup selects extended scanning when the discovered controller
  advertises it, otherwise uses legacy scanning; active scan-response
  accumulation matches Complete Local Name first and Shortened Local Name as
  the fallback, exactly like upstream.
- Classic lookup starts Extended Inquiry mode and matches the same name fields
  in EIR data. A cancelled inquiry completion cannot accidentally restart the
  lookup-owned discovery loop.
- Power-on and bond refresh build a host-side `AddressResolver` even without
  controller offload. Identity lookup matches a directly advertised identity
  or returns the current RPA whose IRK resolves to it.
- Focused tests cover owned and pre-existing scans, concurrent cancellation,
  Classic EIR matching/teardown, rejected no-resolver calls, bonded RPA lookup,
  and the live capability-driven extended-scan branch.

## Slice 177 — what's here

The upstream reference was refreshed from `1d26b998` to `bd64c127`, and its two
new ISO regressions are represented explicitly in the Rust host.

- A zero-SDU CIS direction now forces only its retransmission count to zero.
  The selected PHY remains on the `LE Set CIG Parameters` command, matching the
  current upstream contract and avoiding the reverted zero-PHY behavior.
- BIG and BIG-sync creation reserve their handle until the controller reports
  command status and establishment. A second operation cannot reuse that
  handle while either phase remains pending.
- Non-success command status and final establishment failure both release the
  reservation idempotently and retain the original `(big_handle, status)` in
  the existing error journal, so the same handle can be retried safely.
- Focused tests pin the exact CIG arrays, an immediate controller rejection,
  encrypted BIG-sync establishment failure, duplicate rejection, cleanup, and
  same-handle retry.

## Slice 178 — what's here

Upstream `Host.flush` emits `Device.EVENT_FLUSH`, cancels connection-bound work,
and removes every live connection. The synchronous host now exposes the same
ordered lifecycle explicitly instead of leaving stale state after power-off or
re-power.

- `Device::flush` emits `DeviceEvent::Flush` while connection state is still
  observable, removes every LE, Classic, CIS, and SCO/eSCO handle plus its
  ATT/EATT bearer, L2CAP channel, SMP, encryption, control-request, and ISO
  state, then emits one zero-reason `Disconnected` event per handle.
- Pending name/identity lookup, connection-control, disconnection, Channel
  Sounding, long-term-key, ACL reassembly, and pairing state are cancelled or
  released without discarding already completed result journals.
- `DataPacketQueue::flush_all` accounts for all waiting and in-flight ACL
  packets across every handle, so its lifetime counters and backpressure state
  remain consistent after a transport-wide flush.
- `power_off` flushes whenever the device is powered or connection work remains;
  re-running `power_on` flushes any prior live connection state before reset.
- Focused tests pin listener/journal ordering, multi-transport teardown,
  pending-lookup cancellation, power-off behavior, and exact multi-handle ACL
  accounting.

## Slice 179 — what's here

Upstream `Device.request_remote_name` and `Connection.request_remote_name` now
have a complete event-driven Rust counterpart instead of requiring callers to
construct the raw HCI command and interpret an uncorrelated completion.

- Requests accept either a Classic address or an established ACL handle and
  submit `Remote Name Request` with upstream's R2 page-scan mode, zero reserved
  byte, and zero clock offset.
- Accepted requests remain pending until their address-matched completion;
  immediate Command Status rejection and final HCI failure both release the
  correct request and enter a typed ordered result journal.
- Successful names stop at the first NUL, require valid UTF-8, update
  `ClassicConnectionInfo::peer_name`, retain the compatibility success journal,
  and emit `DeviceEvent::RemoteName`. Malformed UTF-8 and HCI failures emit
  `DeviceEvent::RemoteNameFailure` with their original error detail.
- Flush, re-power, and power-off cancel command correlation and pending request
  counts while preserving already completed remote-name results.
- Focused tests cover exact command defaults, address/handle correlation,
  a live two-controller name exchange, connection-state update, immediate and
  final HCI errors, malformed UTF-8, NUL termination, invalid handles, and
  power-off cancellation.

## Slice 180 — what's here

The reset-time controller discovery in upstream `Host.reset` now extends beyond
the existing Supported Commands and LE feature reads to cover the Classic
version/LMP capability state that `Device.power_on` depends on.

- `Device::reset` flushes an already-ready host, preserves the device powered
  flag, submits HCI Reset, clears stale controller capabilities, and restarts
  Supported Commands discovery.
- When advertised, `Read Local Version Information` retains the complete
  HCI/LMP version tuple and its status. Unsupported controllers leave the
  capability explicitly unknown.
- LMP discovery prefers `Read Local Extended Features`, follows the returned
  maximum through every page, and falls back to the legacy page-zero command.
  Pages, maximum page, and per-page statuses remain queryable; a failed page
  stops the sequence without discarding earlier successful pages.
- Configured interlaced page and inquiry scans are now deferred until LMP
  discovery completes and are enabled independently only when feature bits 29
  and 28 are present, matching upstream's capability checks.
- Focused tests cover version/all-page sequencing, legacy fallback failure,
  mid-sequence extended-page failure, exact scan-mode gating, reset/flush
  ordering, and the live software-controller capability path.

## Slice 181 — what's here

The remainder of upstream `Host.reset`'s event and packet-flow initialization
now runs after reset-time capability discovery reaches a terminal result.

- The host installs upstream's exact Classic event mask, conditionally installs
  the page-2 Encryption Change V2 bit, and chooses either the full current LE
  Meta mask or the conservative five-bit mask required by Bluetooth 4.0 and
  older controllers.
- `ExternalHost::initialize_device` consumes the same exported masks and
  version rule, so physical/system transports cannot retain a stale subset of
  LE events or diverge from the in-process Host reset path.
- Command completion statuses remain queryable and Host initialization becomes
  complete only after every required mask and advertised buffer command has a
  terminal result. Capability-gated Classic scan modes are applied only after
  that initialization succeeds.
- `Read Buffer Size` initializes the Classic/shared ACL pool. LE buffer
  discovery prefers V2, falls back to V1, creates an independent LE pool when
  the controller reports one, and otherwise shares the Classic pool exactly as
  upstream does. V2 also creates the controller-sized ISO pool.
- ACL fragmentation and Number Of Completed Packets routing select the correct
  Classic or LE pool by connection handle. CIS/BIS ISO fragmentation uses the
  discovered packet size and independent flow window; flush and disconnection
  clear queued and in-flight state from all three pools.
- Focused scripted coverage pins the exact masks and command order, old-version
  fallback, V2 and zero-sized V1 results, reset clearing, separate LE window
  draining, and controller-sized ISO window draining. The live software
  controller exercises the same initialization through ordinary `power_on`.

## Slice 182 — what's here

External transports now apply the controller packet pools discovered during
`ExternalHost::initialize_device` without losing the topology learned from HCI.

- A public atomic `Device::configure_controller_packet_pools` retains the raw
  Classic ACL, LE ACL, and ISO descriptors and creates independent queues only
  for pools whose packet length and capacity are both nonzero.
- Zero-capacity LE results share the Classic queue exactly as upstream
  `Host.reset` does, while zero-capacity ISO results remain explicitly
  unavailable. Missing/zero Classic capacity keeps the safe pre-discovery
  fallback rather than creating an unusable queue.
- Reconfiguration refuses to discard queued or in-flight packets from any of
  the three pools. `ExternalHost` now reports that condition instead of
  partially replacing `Device` flow-control state.
- Focused host and external-transport coverage pins distinct 1021/8 Classic,
  251/12 LE, and 120/6 ISO pools plus the zero-sized LE/ISO fallback.

## Slice 183 — what's here

The final capability tail of upstream `Host.reset` now participates in both
in-process and external-controller initialization.

- When both HCI commands are advertised, Host reads the controller's suggested
  default LE data length and writes upstream's 251-octet/2120-microsecond target
  only when either value differs. Read or corrective-write failures make Host
  initialization unsuccessful; matching values avoid the write.
- Supported extended-advertising set count and maximum advertising-data length
  are queried and retained. Their HCI failures remain non-fatal and preserve
  upstream's constructor defaults of zero sets and 31 bytes.
- Initialization completion now waits for every advertised tail command and
  any conditional write, while exposing terminal statuses and effective values
  for deterministic callers.
- `ExternalHost::initialize_device` performs the same ordered reconciliation
  and returns the effective suggestion and advertising capacities in
  `ExternalControllerInfo`.
- Focused scripted tests cover correction, non-fatal advertising-query failure,
  fatal suggested-length read failure, reset clearing, the external command
  sequence, and the live software-controller write path.

## Slice 184 — what's here

The remaining upstream `Host` events that already had typed HCI codecs now
survive the high-level device pump instead of falling through its catch-all.

- Initial Channel Sounding subevent results and their continuation fragments
  retain every status, counter, radio measurement, antenna-path count, and
  per-step payload in separate typed journals.
- Both result forms are published to ordered `DeviceEvent` listeners even when
  the controller reports a handle that is not currently owned by the device,
  matching upstream Host's transport-level event forwarding.
- Opaque vendor events retain their byte-exact payload in a drainable journal
...
## Slice 188 — what's here

The final named transport gap and repository-wide completion audit are closed.

- Serial `dsrdtr` now asserts DTR and gates each outgoing HCI packet on DSR,
  using the configured port timeout and a one-millisecond bounded poll interval
  rather than rejecting the transport spec.
- Pin-read errors remain typed serial errors; an unasserted DSR returns a timed
  out transport I/O error without sleeping past the deadline.
- Transport dispatch was checked against upstream and covers every upstream
  scheme; all Python apps are represented by Rust binaries.
- The Host, Device, and L2CAP tracker rows are complete for the synchronous Rust
  architecture: Python-only awaitables and event-loop scheduling map to explicit
  polling, queues, journals, listeners, and deterministic timers rather than
  missing Bluetooth behavior.
- Focused DSR ready, error, and timeout tests plus the full workspace gates close
  the synchronous Rust port.
