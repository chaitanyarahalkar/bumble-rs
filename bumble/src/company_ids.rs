//! Bluetooth SIG company identifiers, ported from `bumble.company_ids`.
//!
//! A sorted `(company_id, name)` table with a binary-search lookup.

/// All assigned company identifiers, sorted by id.
pub static COMPANY_IDENTIFIERS: &[(u16, &str)] = &[
    (0x0000, "Ericsson AB"),
...
/// The company name for a Bluetooth SIG company identifier, if assigned.
pub fn company_name(company_id: u16) -> Option<&'static str> {
    COMPANY_IDENTIFIERS
        .binary_search_by_key(&company_id, |(id, _)| *id)
        .ok()
        .map(|i| COMPANY_IDENTIFIERS[i].1)
}
...
mod tests {
    ...
    fn lookups_and_sorted() {
        ...
        assert!(COMPANY_IDENTIFIERS.windows(2).all(|w| w[0].0 < w[1].0));
    }
}
